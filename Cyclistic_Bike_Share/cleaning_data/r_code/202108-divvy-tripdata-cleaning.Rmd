---
title: "Cleaning 202108-divvy-tripdata log"
...
## Cyclistic trip data 2021 August
...
input_file <- "202108-divvy-tripdata.csv"
cleaned_tripdata_file <- "2021_08_tripdata.csv"
cleaned_station_file <- "2021_08_stations.csv"
...
valid_start_at <- ymd_hms("2021-08-01 00:00:00")
valid_end_at <- ymd_hms("2021-08-31 23:59:59")
```
...
* started_at data is within the valid date range.
...
```
##### Observations:
* There are 107 records where started_at value is greater than or equal to the ended_at value.
* started_at greater than or equal ended_at records need to be removed from the data set.
...
```
##### Observations:
* monthly_trip_date_v1 contains only duration_in_seconds > 0 records.
...
* The count of unique start station names is 728. 
* The count of unique start station ids is 727. 
...
* The count of unique start station names is 728.
* The count of unique start station ids is 728.
...
```
##### Observations:
* There are 706 entries with null values for end_latitude and end_longitude fields


```{r}
monthly_trip_data_v2 <- monthly_trip_data_v1 %>% filter(!is.na(start_latitude) | !is.na(start_longitude))
